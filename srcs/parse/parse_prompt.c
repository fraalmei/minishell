...
{
	printf("dpnde rebienta %c %s\n", buffer[*i], swap->command);
	if ((buffer[*i] == '-') && (buffer[*i + 1] != ' ') && !swap->arguments[2])
	{
		printf("dpnde rebienta 1 \n");
...
/* static int	ride_buffer(char *buff, t_prompt **prom, t_prompt **swap, int *i)
{
	...
	if (!(*swap)->command && buff[*i] != ' ' && is_redirecction(&buff[*i]) == 0)
	{
		...
		printf("pasa command %s %s\n", (*swap)->command, (*swap)->arguments[0]);
